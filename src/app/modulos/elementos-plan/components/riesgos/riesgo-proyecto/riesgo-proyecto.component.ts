...
@Component({
  selector: 'app-riesgo-proyecto',
  templateUrl: './riesgo-proyecto.component.html',
  styleUrls: ['./riesgo-proyecto.component.css']
})
export class RiesgoProyectoComponent implements OnInit, OnDestroy {
  ...
  private buffer!: any;
  public nameModalRiesgoP: string = 'modalRiesgoP'
  public nameModalGestionRP: string = 'modalGestionRP'
  public nameAppP = 'Riesgos de Proyectos';
  public nameAppGestionRP = 'Acciones de Riesgos';
  public selectItems: any = [];
  private idSprAmbiente: number = 0;
  public dataRiesgo: any[] = [];
  public dataProyectoRiesgo: any = [];
  ...
  getDataProyecto() {
    this.proyectoService.list().subscribe((resp) => {
      this.dataProyecto = resp;
      console.log('estrategias: ', this.dataProyecto);
  ...
  mapeoRiesgoLinea() {
    //this.dataProyectoRiesgo = []
    let dataProyectoMap: any =[];
    let indiceUno = 0;
    ...
    for (let i = 0; i < this.dataObjetivo.length; i++) {
      for (let j = 0; j < this.dataEstrategiaObjetivo.length; j++) {
        if ((this.dataObjetivo[i].idSprObjetivo == this.dataEstrategiaObjetivo[j].idSprObjetivo)&& (this.dataEstrategiaObjetivo[j].nivelSuperior === 'N')) {
          console.log("idSprObjetivo1",this.dataObjetivo[i].idSprObjetivo);
          console.log("idSprObjetivo2",this.dataEstrategiaObjetivo[j].idSprObjetivo);

          for (let l = 0; l < this.dataProyecto.length; l++) {
            if ((this.dataProyecto[l].idSprEstrategia == this.dataEstrategiaObjetivo[j].idSprEstrategia)&& (this.dataProyecto[l].idSprObjetivo == null)) {

              for (let k = 0; k < this.dataRiesgo.length; k++) {
              if (this.dataRiesgo[k].idSprProyecto == this.dataProyecto[l].idSprProyecto) {
                ...
                dataRiesgoLineaMap.push({
                  idSprProyecto: this.dataProyecto[l].idSprProyecto,
                  lineaAccion: this.dataProyecto[l].nombreProyecto,
          ...
          else if ((this.dataProyecto[l].idSprObjetivo == this.dataEstrategiaObjetivo[j].idSprObjetivo)&& (this.dataProyecto[l].idSprEstrategia == null)){
            
            for (let k = 0; k < this.dataRiesgo.length; k++) {
              if (this.dataRiesgo[k].idSprProyecto == this.dataProyecto[l].idSprProyecto) {
                console.log("proyectos",this.dataRiesgo[k].idSprRiesgo);
          console.log("estrateOB",this.dataRiesgo[k].nombre);
                dataRiesgoLineaMap.push({
                  idSprProyecto: this.dataProyecto[l].idSprProyecto,
                  lineaAccion: this.dataProyecto[l].nombreProyecto,
                  ambiente: this.dataObjetivo[i].idSprAmbiente,
    ...
    console.log("dataProyectoMap",dataProyectoMap)
    this.dataProyectoRiesgo =  dataProyectoMap
  }

getRiesgosAmbiente(){  ///metodo en construcción prueba
  forkJoin(
  ...
  setFromObjetivo(idSprProyecto: string = " ") {
    console.log('Modal row...', idSprProyecto)
  ...
  delete(id: number) {
    ...
    this.modal.open('delRiesgoProyecto');
  }
...
this.modal.close();
  ...
  edit(obj: Riesgos) {
    ...
    //this.idLineaAccion = obj.idSprProyecto;
    this.modal.open(this.nameModalRiesgoP);
  }
  ...
  openModal(row: any) {
    this.setFromObjetivo(row.idSprProyecto);
    this.modal.open(this.nameModalRiesgoP);
  }
  openModalGestion(row: any) {
    ...
    console.log('Row: ', row);
    this.idRiesgoSeleccionado = row.idSprRiesgo
    this.modal.open(this.nameModalGestionRP);
  }
