...
@Component({
  selector: 'app-riesgo-plan-accion',
  templateUrl: './riesgo-plan-accion.component.html',
  styleUrls: ['./riesgo-plan-accion.component.css']
})
export class RiesgoPlanAccionComponent implements OnInit, OnDestroy {
  ...
  private buffer!: any;
  public modalRiesgoPlanAccion: string = 'modalRiesgoPlanAccion'
  public modalGestionPlanAccion: string = 'modalGestionPlanAccion'
  public nameApp = 'Riesgos de Planes de Acción';
  ...
  listObjetivos() {
    this.objetivoService.list(`?idSprAmbiente=${this.idSprAmbiente}`).subscribe((resp) => {
      ...
      console.log('dataObjetivo ', this.dataObjetivo);
  ...
  getDataLineaAccion() {
    this.planAccionService.list().subscribe((resp) => {
      ...
      console.log('dataPlanAccion ', this.dataPlanAccion);
  ...
  mapeoRiesgoLinea() {
          ...
          for (let j = 0; j < this.dataPlanAccion.length; j++) {
            if (this.dataObjetivo[i].idSprObjetivo == this.dataPlanAccion[j].idSprObjetivo) {
              ...
              for (let k = 0; k < this.dataRiesgo.length; k++) {
                if (this.dataRiesgo[k].idSprPlanAccion == this.dataPlanAccion[j].idSprPlanAccion) {
                      ...
                      for (let x = 0; x < this.dataEquipoGerencial.length; x++) {
                        if ((this.dataEquipoGerencial[x].idSprEquipoGerencial == this.dataPersonalElemento[w].idSprEquipoGerencial)&&(this.dataPersonalElemento[w].estado==1)) {
                          ...
                          dataRiesgoLineaMap.push({
                            ...
                            planAccion: this.dataPlanAccion[j].nombre,
    ...
    console.log("dataPlanAccionMap", dataLineaAccionMap)
  ...
  setFromObjetivo(idSprPlanAccion: string = " ") {
    console.log('Modal row...', idSprPlanAccion)
  ...
  delete(id: number) {
    ...
    this.modal.open('delPlanAccion');
  }

  confirmDelete() {
    console.log("idDeleteRegister",this.idDeleteRegister)
...
this.modal.close();
  ...
  edit(obj: Riesgos) {
    ...
    //this.idLineaAccion = obj.idSprPlanAccion;
    this.modal.open(this.modalRiesgoPlanAccion);
  }
  ...
  openModal(row: any) {
    console.log('Row: ', row);
    //this.idLineaAccionSeleccionado = row.idSprPlanAccion;
    this.setFromObjetivo(row.idSprPlanAccion);
    this.modal.open(this.modalRiesgoPlanAccion);
  }
  openModalGestion(row: any) {
    ...
    this.modal.open(this.modalGestionPlanAccion);
  }
