...
@Component({
  selector: 'app-accion-proyecto',
  templateUrl: './accion-proyecto.component.html',
  styleUrls: ['./accion-proyecto.component.css']
})
export class AccionProyectoComponent implements OnInit, OnDestroy{
  ...
  public nameModalAccionR: string = 'modalNewAccionRProyecto';
  ...
  confirmDelete() {
    if (this.idDeleteRegister > 0) {
      this.accionService.delete(      
        ...
        .subscribe(resp => {
          //this.initApp();
          this.modalAccion.closeId('delAciones')   
         this.alert.deleteOk(); 
        }, (err) => {
