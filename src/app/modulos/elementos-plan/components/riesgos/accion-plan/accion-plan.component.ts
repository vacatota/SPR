...
@Component({
  selector: 'app-accion-plan',
  templateUrl: './accion-plan.component.html',
  styleUrls: ['./accion-plan.component.css']
})
export class AccionPlanComponent implements OnInit, OnDestroy{
  ...
  public modalAccionPlan: string = 'modalAccionPlan';
      ...
      if (this.formAccion.value.idSprAccion == null) {
        this.accionService.create(this.formAccion.value).subscribe(
          (resp) => {
            //this.initApp();
            this. getDataAcciones();
            this.modalAccion.closeId(this.modalAccionPlan);
            this.alert.createOk();
          },
          (err) => {
  ...
  openModalAccion() {
    this.setForm();
    //this.onResetForm();
    console.log("this.modalAccionPlan",this.modalAccionPlan)
    this.modalAccion.open(this.modalAccionPlan);
  }
