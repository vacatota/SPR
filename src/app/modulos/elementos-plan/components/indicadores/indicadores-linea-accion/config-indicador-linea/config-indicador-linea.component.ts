...
@Component({
  selector: 'app-config-indicador-linea',
  templateUrl: './config-indicador-linea.component.html',
  styleUrls: ['./config-indicador-linea.component.css']
})
export class ConfigIndicadorLineaComponent {
  public formConfigIndicadoLinea!: UntypedFormGroup;
  public formSubmitted = false;
  public currentAmbiente: any;
  public ipAddress!: string;
  public buffer: any;
  public dataConfigIndicadorLinea: any[] = [];
  ...
  )
  {
    ...
    get procesoForm() {
      return this.formConfigIndicadoLinea.controls;
    }
    ...
    setForm() {
    this.formConfigIndicadoLinea = this.fb.group({
  ...
  initApp() {
   ...
   // this.rangoIntermedio();
    ...
    listConfiguracionIndicadores(idSprIndicador: any) {
      console.log('idSprIndicador',idSprIndicador);
      this.configuracionIndicadoresService.list('?idSprIndicador=' + idSprIndicador)
        .subscribe(resp => {
          this.dataConfigIndicadorLinea = resp;
          console.log('ConfigIndicadores: ', this.dataConfigIndicadorLinea);
  ...
  onSubmit(){
    ...
    this.formConfigIndicadoLinea.value.idSprIndicador =this.idSprIndicador;
    this.formConfigIndicadoLinea.value.usuario = this.currentAmbiente.idSprUsuario;
    this.formConfigIndicadoLinea.value.ip = this.ipAddress;
  ...
  //  this.formConfiguracionIndicadores.value.rangoIntermedio = this.rangoTotal;
    console.log('grabar/EDITAR: ', this.formConfigIndicadoLinea.value);
console.log('guardoooo',this.formConfigIndicadoLinea.invalid);
    if (this.formConfigIndicadoLinea.invalid) {
      if (this.formConfigIndicadoLinea.value.idSprConfigIndicadores == null) {
this.configuracionIndicadoresService.create(this.formConfigIndicadoLinea.value).subscribe(
  (resp) => {
    this.initApp();
...
this.configuracionIndicadoresService.update(this.formConfigIndicadoLinea.value).subscribe(
  (resp) => {
    this.initApp();
    this.closeModalIndicador();
    this.alert.updateOk();
...
return false;
  ...
  edit(obj: ConfiguracionIndicadores) {
    ...
    this.formConfigIndicadoLinea.patchValue({
...
//################################################
 ...
 //#################################
  ...
  rangoIntermedio() {
    ...
    let menorIgual: any;
    let totalRango: any;
    mayorIgual = this.formConfigIndicadoLinea.value.mayorIgual;
    console.log('mayor',mayorIgual);
   // if((mayorIgual >= 100) && (menorIgual <= 85)){
    menorIgual = this.formConfigIndicadoLinea.value.menorIgual;
  ...
  onResetForm() {
    ...
    this.formConfigIndicadoLinea.reset();
}
closeModalIndicador(){
